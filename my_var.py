    ...
    def forward(self,
        ...
        for stage_num in range(STAGE_TOTAL):
            ...
            for idx in range(HEAD_NUM):
                ...
                hidden_silu = self.ada_lin_silu(input_ids)
                ...
                qkv = self.mat_qkv_linear(input_ids)
                self.generator.linear_layer(
                    # name
        ...
        # qkv: BL3Hc  16 1(L) 3 16 64
                ...
                fc1 = self.fc1_linear(attn_output)
                self.generator.linear_layer(
                    # name
                    layer_name          = name_prefix + "fc1_layer" + name_suffix,
                    # input
                    input_name          = name_prefix + "attn_output" + name_suffix,
            ...
            device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
            Phi_input = torch.zeros((B,CVAE,W,H)).to(device)
