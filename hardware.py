...
class HBM():
    def __init__(self) -> None:
        ...
        self.end_time_list = []
        self.time_stamp = {'start_time':{},'end_time':{}}
    def load_data(self,compute_end_time:int,data_amount:int,dstensor0:Data_Split_Tensor):
        if dstensor0.id not in self.time_stamp['start_time']:
            self.time_stamp['start_time'][dstensor0.id] = [] 
            self.time_stamp['end_time'][dstensor0.id] = [] 
        ...
        self.start_time_list.append(start_time);self.time_stamp['start_time'][dstensor0.id].append(start_time)
        self.end_time_list.append(self.last_access_end_time);self.time_stamp['end_time'][dstensor0.id].append(self.last_access_end_time)
...
class Bank():
    ...
    def read_data(self,compute_end_time:int, dtensor:Data_Split_Tensor): 
        self.access_flag += 1
        if dtensor.id not in self.time_stamp['start_time']:
            self.time_stamp['start_time'][dtensor.id] = [] 
            self.time_stamp['end_time'][dtensor.id] = [] 
            
        if self.type == 'FI':
            start_time = max(self.last_access_end_time,compute_end_time)
            duration_time = dtensor.shape[0] * dtensor.shape[1] * 5# TODO：具体时间之后推一下
            self.last_access_end_time = start_time + duration_time
            self.start_time_list.append(start_time);self.time_stamp['start_time'][dtensor.id].append(start_time)
            self.end_time_list.append(self.last_access_end_time);self.time_stamp['end_time'][dtensor.id].append(self.last_access_end_time)
    ...
    #从计算单元写data到buffer
    def write_data(self,end_time:int,dtensor:Data_Split_Tensor): 
        if dtensor.id not in self.time_stamp['start_time']:
            self.time_stamp['start_time'][dtensor.id] = [] 
            self.time_stamp['end_time'][dtensor.id] = [] 
        if self.type == 'FO':
            start_time = max(self.last_access_end_time,end_time)
            duration_time = dtensor.shape[0] * dtensor.shape[1] * 5 # TODO:把这个东西写成dtensor.ammount
            self.last_access_end_time = start_time + duration_time
            self.start_time_list.append(start_time);self.time_stamp['start_time'][dtensor.id].append(start_time)
            self.end_time_list.append(self.last_access_end_time);self.time_stamp['end_time'][dtensor.id].append(self.last_access_end_time)
        elif self.type == 'Param':
...
class NoC_Router():
    ...
    def transfer_data(self, ready_time:int,dstensor0:Data_Split_Tensor,src_tile:Tile, dst_tile:Tile,hardware:'Hardware'):
        ...
        transfer_time = 10000 * noc_distance * dstensor0.amount / self.NoC_bandwidth # TODO: 需要check
...
class MFU():
    ...
    def process_silu(self,dstensor0: Data_Split_Tensor,result_tensor: Data_Split_Tensor,tile_execute:Tile, hardware:'Hardware',op:str):
        hbm = hardware.HBM
        buffer = tile_execute.Buffer #应该传进来一个tile
        mfu = tile_execute.MFU
        # 每次启用tile的时候需要做的，似乎这里也ok，因为没有重新例化bank，只不过给bank分类了

        buffer.FI_bank_num, buffer.Parameter_bank_num, buffer.FO_bank_num = FI_bank_num, Param_bank_num, FO_bank_num = 4,0,4 # 这里应该是上一行的函数给一个结果，这里先随便设的
        #Param_dram = 4
        ...
        for bank in FI_bank[buffer.FI_access_flag % 2]:
            # 应该是另一组bank正在计算的结果
            if len(mfu.end_time_list) == 0 : FI_ready_time = bank.read_data(0,dstensor0) #这里在传其他tile_id的时候就不能是0了，因为hbm顺移，它在第二次推FIreadytime的时候起始时间就变得很大
            else: FI_ready_time = bank.read_data(mfu.end_time_list[-1],dstensor0)
        buffer.FI_access_flag += 1 # 用于pingpong
        
        FO_compute_ready_time = mfu.compute_silu(dstensor0,FI_ready_time,'silu')

        for bank in FO_bank[0]:
            FO_write_ready_time = bank.write_data(FO_compute_ready_time,result_tensor)
    
    
    def process_MM(self,dstensor0: Data_Split_Tensor, dstensor1: Data_Split_Tensor,result_tensor: Data_Split_Tensor,tile_execute:Tile, hardware:'Hardware',op:str):
        ...
        for bank in FI_bank[buffer.FI_access_flag % 2]:
            # 应该是另一组bank正在计算的结果
            if len(mfu.end_time_list) == 0 or len(mfu.end_time_list) == 1: FI_ready_time = bank.read_data(0,dstensor0) #这里在传其他tile_id的时候就不能是0了，因为hbm顺移，它在第二次推FIreadytime的时候起始时间就变得很大
            else: FI_ready_time = bank.read_data(mfu.end_time_list[-2],dstensor0)
        buffer.FI_access_flag += 1 # 用于pingpong

        if len(mfu.end_time_list) == 0 or len(mfu.end_time_list) == 1: Param_load_time = hbm.load_data(0,Param_dram,dstensor0) # input 的 情况
        else: Param_load_time = hbm.load_data(mfu.end_time_list[-2],Param_dram,dstensor0)
        ...
        FO_compute_ready_time = mfu.compute_MM(dstensor0,dstensor1,FI_ready_time,Param_ready_time,'mat_mat_mul')
        ...
        # 需要加上滞留的FO
            ...
            FO_add_ready_time = mfu.compute_MM(result_tensor,result_tensor,FO_read_ready_time,FO_compute_ready_time,'mat_mat_add')
    ...
    def merge(self,dstensor0: Data_Split_Tensor,src_tile:Tile,dst_tile:Tile,hardware:'Hardware'):
        ...
        for bank in src_FO_bank[0]:
            if len(hardware.NoC_router.end_time_list) == 0: src_FO_read_ready_time = bank.read_data(0,dstensor0)
            else: src_FO_read_ready_time = bank.read_data(hardware.NoC_router.end_time_list[-1],dstensor0)# 改一下读取时间应该是noc通信完毕的时间
        for bank in dst_FO_bank[0]:
            if len(hardware.NoC_router.end_time_list) == 0: dst_FO_ready_time = bank.last_access_end_time# TODO:CHECK一下，第一次是dst本身FO准备好的时间
            else: dst_FO_ready_time = dst_tile.MFU.end_time_list[-1] # 第二次应该是前一个merge计算结束的时间，省略：这个fo应该是固定的一个值，同上一行结果因为这里采用的是先计算，后统一整合merge else情况中src_FO_read_ready_time>dst_FO_ready_time所以它不重要了，这里就置为0了
        # 从src通信到dst
        ...
        FO_add_ready_time = dst_tile.MFU.compute_MM(dstensor0,dstensor0,FI_read_ready_time,dst_FO_ready_time,'mat_mat_add')
        
        for bank in dst_FO_bank[0]:
            FO_write_ready_time = bank.write_data(FO_add_ready_time,dstensor0)
        
    def compute_silu(self,dstensor0: Data_Split_Tensor,FI_ready_time,op):
        if dstensor0.id not in self.time_stamp['start_time']:
            self.time_stamp['start_time'][dstensor0.id] = [] 
            self.time_stamp['end_time'][dstensor0.id] = [] 
        # else:
        #     print(1)
        if op == 'silu':
            start_time = max(self.last_access_end_time,FI_ready_time)
            duration_time = dstensor0.shape[0] * dstensor0.shape[1] /5
            self.last_access_end_time = start_time + duration_time
            self.start_time_list.append(start_time);self.time_stamp['start_time'][dstensor0.id].append(start_time)
            self.end_time_list.append(self.last_access_end_time);self.time_stamp['end_time'][dstensor0.id].append(self.last_access_end_time)
        return self.last_access_end_time
    
    def compute_MM(self,dstensor0: Data_Split_Tensor,dstensor1: Data_Split_Tensor,FI_ready_time,Param_ready_time,op):
        if dstensor0.id not in self.time_stamp['start_time']:
            self.time_stamp['start_time'][dstensor0.id] = [] 
            self.time_stamp['end_time'][dstensor0.id] = [] 
        # else:
...
class Hardware:
    ...
    def process(self,dstensor0: Data_Split_Tensor, dstensor1: Data_Split_Tensor,result_tensor: Data_Split_Tensor,tile_execute:Tile,op:str):
        if op == "MM":
            tile_execute.MFU.process_MM(dstensor0,dstensor1,result_tensor,tile_execute,self,op)
        elif op == 'silu':
            tile_execute.MFU.process_silu(dstensor0,result_tensor,tile_execute,self,op)
